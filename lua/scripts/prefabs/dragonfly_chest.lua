...
local prefabs =
{
    "collapse_small", --生成摧毁效果
    "chestupgrade_stacksize_taller_fx", --箱子升级效果
    "alterguardianhatshard", --可能是箱子的掉落物
	"collapsed_dragonflychest", --可能是箱子摧毁后的效果
}

local function ShouldCollapse(inst) --是否塌陷
...
local function ConvertToCollapsed(inst, droploot) --转成塌陷
...
local function upgrade_onhammered(inst, worker)--升级后的毁坏
...
local function upgrade_onhit(inst, worker)--升级后的击打
	if inst.components.container ~= nil then
...
local function DoUpgradeVisuals(inst)--升级后的视觉效果
...
local function OnUpgrade(inst, performer, upgraded_from_item)--升级
