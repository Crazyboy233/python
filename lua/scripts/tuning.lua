...
function Tune(overrides)
    ...
    TUNING =
    {
		...
		ARMOR_LUNARPLANT_PLANAR_DEF = 10, --默认10点基础位面防御（抵挡10点位面伤害）
        -- 表示这两种护甲的头盔部分，耐久性和吸收率与身体护甲部分相同。
		...
		ARMOR_LUNARPLANT_LUNAR_RESIST = 0.9,--亮茄套装提供90%的月之抗性
        --下面表示当玩家穿戴这两个套装的两件装备时，提供额外的抗性。
        --通过 math.sqrt(0.75) / 0.9 计算得出，因为两个部件组合提供更高的抗性。
		ARMOR_LUNARPLANT_SETBONUS_LUNAR_RESIST = math.sqrt(0.75) / 0.9, --sqrt because two pieces combine to achieve this
		--下面为植物人的亮茄甲的反伤：正常为10点位面伤害，对暗影生物为20点位面伤害
		...
		ARMOR_VOIDCLOTH_PLANAR_DEF = 10, --默认10点基础位面防御（抵挡10点位面伤害）
